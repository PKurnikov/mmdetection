...
class TestDFINE(TestCase):

    def setUp(self) -> None:
        register_all_modules()

    def test_dfine_head_loss(self):
        ...
        batch_data_samples = [img_metas] * batch_size
        # for i in range(batch_size):
        #     batch_data_samples.append(img_metas)

        config = get_detector_cfg('dfine/cfg_dfine.py')

        model = MODELS.build(config)
        model.init_weights()
        random_image = torch.rand(batch_size, 3, s, s)
        ...
        batch_data_samples1 = [img_metas] * batch_size
        ...
        img_metas.gt_instances = gt_instances
        batch_data_samples2 = [img_metas] * batch_size
